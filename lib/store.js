...
const mem = { tasks: new Map(), events: new Map(), inbox: new Map(), claimed: new Set() };
...
// Returns true the first time a key is claimed within ttlSeconds, false afterwards.
export async function claimOnce(key, ttlSeconds) {
  const r = await redis();
  if (!r) {
    if (mem.claimed.has(key)) return false;
    mem.claimed.add(key);
    return true;
  }
  return (await r.set(`${PREFIX}once:${key}`, '1', { NX: true, EX: ttlSeconds })) === 'OK';
}
