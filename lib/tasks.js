...
export function nextAction(task) {
  ...
  switch (task.status) {
    ...
    default:
      if (task.status === 'queued' && quiet >= 5) {
        return `The session has not sent a single report in ${quiet} min. Open session_url: it may still be starting, or it did not follow the orchestrator protocol (the routine's own prompt must tell it to). Keep polling every ~${POLL_SECONDS}s.`;
      }
