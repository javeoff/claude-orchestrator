...
// MCP clients authenticate with MCP_AUTH_TOKEN (Bearer header or ?key= in the URL)
// or with an access token issued by the built-in OAuth flow (claude.ai connectors).
export function checkMcpAuth(request) {
  ...
  if (bearer && (safeEqual(bearer, expected) || isValidAccessToken(bearer))) return true;
  return Boolean(key) && safeEqual(key, expected);
}
